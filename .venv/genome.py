...
class Genome():
    ...
    # apply point mutation with different rates for different parts types
    @staticmethod
    def targeted_point_mutate(dna, rates, amount):
        new_dna = copy.deepcopy(dna)
        for part in new_dna:
            part_type = part['type']
            # set default value to 0 if type is not in rates
            rate = rates.get(part_type, 0.0)
            
            for i in range(len(part['gene'])):
                if random.random() < rate:
                    # mutate the gene
                     part['gene'][i] += (random.random() - 0.5) * amount 
                if part['gene'][i] >= 1.0:
                    part['gene'][i] = 0.9999
                if part['gene'][i] < 0.0:
                    part['gene'][i] = 0.0
    ...
    def targeted_grow_mutate(dna, rate):
        # add a new leg part but never add a new torso
        if random.random() < rate:
            new_dna = copy.deepcopy(dna)
            spec_len = len(new_dna[0]['gene'])
            new_dna.append({'type': creature.PartType.LEG, 'gene': Genome.get_random_gene(spec_len)})
            return new_dna
        return copy.deepcopy(dna)
    
    @staticmethod
    def targeted_shrink_mutate(dna, rate):
        # remove a random leg part, but never remove a torso
        # find indinces for all leg parts
        leg_indices = [i for i, part in enumerate(dna) if part['type'] == creature.PartType.LEG]
        
        if not leg_indices:
            # cannot shrink if there are no leg parts
            return copy.deepcopy(dna)
        
        if random.random() < rate:
            new_dna = copy.deepcopy(dna)
            # choose a random leg to remove
            ind_to_remove = random.choice(leg_indices)
            del new_dna[ind_to_remove]
            return new_dna
        return copy.deepcopy(dna)
    
    @staticmethod
    def targeted_crossover(dna1, dna2):
        # perform crossover on each part, assume one torso
        new_dna = []
        
        # crossover for the torso, assuming the first element is a torso
        torso1 = dna1[0]['gene']
        torso2 = dna2[0]['gene']
        
        # crossover for the torso  
        x_point = random.randint(1, len(torso1)-1)
        new_torso_gene = np.concatenate((torso1[x_point:], torso2[:x_point]))
        new_dna.append({'type': creature.PartType.TORSO, 'gene': new_torso_gene})
        
        # crossover for the leg 
        legs1 = [p['gene'] for p in dna1 if p['type'] == creature.PartType.LEG]
        legs2 = [p['gene'] for p in dna2 if p['type'] == creature.PartType.LEG]
        
        num_legs_new = random.choice([len(legs1), len(legs2)])
        # if there are no legs, return the first parent
        if num_legs_new == 0:
            return dna1
        
        for i in range(num_legs_new):
            # choose a leg from either parent
            parent_leg_gene = random.choice([legs1[i% len(legs1)], legs2[i% len(legs2)]])
            new_dna.append({'type': creature.PartType.LEG, 'gene': copy.copy(parent_leg_gene)})
